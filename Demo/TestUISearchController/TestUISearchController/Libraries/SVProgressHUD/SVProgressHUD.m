...
+ (void)showImage:(UIImage *)image status:(NSString *)string maskType:(SVProgressHUDMaskType)maskType {
    ...
    [[self sharedView] showImage:image status:string duration:1 maskType:maskType];
}
    ...
    [UIView animateWithDuration:0.15
                     ...
                     completion:^(BOOL finished){
                         if(self.alpha == 0.0f || self.hudView.alpha == 0.0f) {
                             ...
                             //QLLog(@"%@", [UIApplication sharedApplication].windows);
                             //QLLog(@"keyWindow = %@", [UIApplication sharedApplication].keyWindow);
                         }
                     }];
}
